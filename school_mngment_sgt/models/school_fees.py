...
class StudentPayslip(models.Model):
    ...
    def invoice_view(self):
        """View number of invoice of student"""
        invoice_obj = self.env['account.move'].search([("student_payslip_id", "=", self.id)])
        action = self.env.ref('account.action_move_out_invoice_type').sudo().read()[0]
        if len(invoice_obj) > 1:
            action['domain'] = [('id', 'in', invoice_obj.ids)]
        elif len(invoice_obj) == 1:
            action['views'] = [(self.env.ref('account.view_move_form').id, 'form')]
            action['res_id'] = invoice_obj.ids[0]
        else:
            action = {'type': 'ir.actions.act_window_close'}
        return action
